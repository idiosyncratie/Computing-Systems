...
rl_decode:
        ...
        xor     %edx, %edx                # buffer between adding bytes
...
find_space_loop:
                                        # Error 1: Add in the count, then move
        mov    (%ecx, %esi), %dl         # get one byte
        add     %edx, %ebx                # add for total computed size
        add     $2, %esi                  # Error 2: 
                                          # forward to the next count!
...
find_space_done:
        ...
        call    malloc            # Error 3: Push ecx before malloc then pop
...
write_loop:
        mov     %bl, (%eax, %edi)
        inc     %edi            # error 4: increment where you write
        dec     %bh
        jnz     write_loop
