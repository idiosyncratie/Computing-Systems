...
void Shape_class_init(Shape_Class *class) {
    class->getVolume = NULL; // initialize getVolume
}
...
void Shape_init(Shape_Data *this, Shape_Class *class, float D) {
    this->class = class;
    Shape_setDensity(this, D); // set density
}
...
void Box_class_init(Box_Class *class) {
    // initialize get volume and connect to shape
...
void Box_init(Box_Data *this, Box_Class *class,
    ...
    // initialize values and shape class for Box
...
Box_Data * new_Box(float L, float W, float H, float D) {
    // make a box and set initialize it to box
    Box_Data* temp = (Box_Data*)malloc(sizeof(Box_Data));
    Box_init(temp, &Box, L, W, H, D);
    return temp;
}
...
void Sphere_init(Sphere_Data *this, Sphere_Class *class, float R, float D) {
    // only need to set radius because density set in shape
    Shape_init((Shape_Data *)this, (Shape_Class *)class, D);
...
Sphere_Data * new_Sphere(float R, float D) {
    // malloc sphere and initialize as sphere
    Sphere_Data* temp = (Sphere_Data*)malloc(sizeof(Sphere_Data));
    Sphere_init(temp, &Sphere, R, D);
    return temp;
}
...
void Cone_init(Cone_Data *this, Cone_Class *class, float BR, float H, float D) {
    // only need to set base radius and height because density set in shape
    Shape_init((Shape_Data *)this, (Shape_Class *)class, D);
...
Cone_Data * new_Cone(float BR, float H, float D) {
    // malloc cone and initialize as cone
    Cone_Data* temp = (Cone_Data*)malloc(sizeof(Cone_Data));
    Cone_init(temp, &Cone, BR, H, D);
    return temp;
}
