...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    p_cache->num_reads++;
    /* Update the most recent access time when a byte is read */
    p_line->recTime = clock_tick();
...
void cache_write_byte(membase_t *mb, addr_t address, unsigned char value) {
    ...
    p_line->dirty = 1;
    /* Update the most recent access time when a byte is written */
    p_line->recTime = clock_tick();
}
...
#if DEBUG_CACHE
    printf(" * Decomposed address %x into tag %x, set %x, and offset %x\n",
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    // mask for block size and bit shift to get the parameters

    *offset = (p_cache->block_size - 1) &  address;
    *set = (p_cache->num_sets - 1) & (address >> p_cache->block_offset_bits);
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
// use a mask to block the offset 
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
// concatenate the tag then set_no and shift left by the num 
// of bits for the offset   
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    for (int curr = 0; curr < p_set->num_lines; ++curr) {
        c_line = p_set->cache_lines + curr;
        if (c_line->valid == 1) {
                if (c_line->tag == tag) {
            found_line = c_line;
            }
...
cacheline_t * choose_victim(cacheset_t *p_set) {
    ...
    cacheline_t *min_line = p_set->cache_lines + 0;
    // iterate through cache lines
    for (i_victim = 0; i_victim < p_set->num_lines; ++i_victim) {
        victim = p_set->cache_lines + i_victim;
        // line is empty, so return immediately
        if (victim->valid == 0) {
                return victim;
        } else {
            if (victim->recTime < min_line->recTime) {
                min_line = victim;
            }
        }
    }
